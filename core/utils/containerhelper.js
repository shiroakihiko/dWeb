...
class ContainerHelper {
    ...
    static generateHash(block) {
        ...
        const blockData = ContainerHelper.removeSignatures(block);

        // Sort the object keys and stringify it in a canonical way
        const canonicalData = ContainerHelper.canonicalStringify(blockData);
    ...
    static signBlock(privateKey, block) {
        ...
        return signer.signMessage(ContainerHelper.canonicalStringify(blockWithoutSignatures));
    }
    ...
    static verifySignatureWithPublicKey(block, signatureBase64, publicKey) {
        ...
        const blockWithoutSignatures = ContainerHelper.removeSignatures(block);

        // Serialize the block (ensure the same format as in signing)
        const serializedBlock = ContainerHelper.canonicalStringify(blockWithoutSignatures);
    ...
    static mergeSignatures(block, newBlock) {
        ...
        Object.entries(newBlock.validatorSignatures).forEach(([sigNodeId, signature]) => {
            ...
            if (!block.validatorSignatures.hasOwnProperty(sigNodeId)) {
                ...
                const isValid = ContainerHelper.verifySignatureWithPublicKey(block, signature, sigNodeId);
    ...
    // Container Signing
    static signContainer(container, privateKey) {
        const signer = new Signer(privateKey);
        return signer.signMessage(container.hash);
    }

    // Container Signature Verification
    static verifySignatureWithPublicKey(containerHash, signatureBase64, publicKey) {    
        return Signer.verifySignatureWithPublicKey(containerHash, signatureBase64, publicKey);
    }

    static generateContainerHash(container) {
        const blockContainer = new BlockContainer();
        blockContainer.previousContainerHash = container.previousContainerHash;
        blockContainer.blocks = container.blocks;
        blockContainer.timestamp = container.timestamp;
        blockContainer.creator = container.creator;
        blockContainer.validatorSignatures = container.validatorSignatures;
        return blockContainer.calculateHash();
    }
}
