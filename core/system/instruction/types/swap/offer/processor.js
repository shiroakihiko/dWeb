...
class OfferInstructionProcessor extends BaseInstructionProcessor {
    constructor(network) {
        super(network);
    }
    ...
    async processInstruction(processData) {
        ...
        if (instruction.type !== 'swapOffer') {
            throw new Error('Invalid instruction type for OfferInstructionProcessor');
        }
        ...
        {
            this.network.ledger.actionCallbacks.addCallback(action.hash, 'swapOffer');
        }
...
module.exports = OfferInstructionProcessor;