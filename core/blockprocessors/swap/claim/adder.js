    ...
    async addBlock(block, containerHash) {
        try {
            return await this.network.ledger.blocks.transaction(async () => {
                ...
                const accountClaimer = await accountManager.getAccountUpdate(block.fromAccount);
                ...
                accountClaimer.updateBalance(new Decimal(accountClaimer.getBalance()).plus(accountSwap.getBalance()).toString());
                accountClaimer.setBlockCountIncrease(1);
                accountClaimer.updateLastBlockHash(block.hash);
...
module.exports = ClaimBlockAdder;